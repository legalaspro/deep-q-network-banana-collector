...
from standrard_model import QNetwork
...
class Agent():
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        if self.is_prioritzed:
            # td_error = self.get_single_error(state, action, reward, next_state, done)
    ...
    def get_single_error(self, state, action, reward, next_state, done):
        """
            Calculate TD error for a single experience tuple.    
        Params:
        -------
            state (np.array or list): Current state.
            action (int): Action taken in the current state.
            reward (float): Reward received after taking action.
            next_state (np.array or list): Next state after taking the action.
            done (bool): Whether the episode has ended.
        Returns:
        --------
            float: TD error for prioritization.
        """
        # Convert inputs to tensors and reshape as needed
        state = torch.from_numpy(state).float().unsqueeze(0).to(device)
        next_state = torch.from_numpy(next_state).float().unsqueeze(0).to(device)
        action = torch.tensor(action, dtype=torch.long, device=device)  # single integer, no need for [[action]]
        reward = torch.tensor(reward, dtype=torch.float32, device=device)
        done = torch.tensor(done, dtype=torch.float32, device=device)

        # Double DQN logic for Q target calculation
        with torch.no_grad():
            # Get the best next action for double DQN
            best_next_action = self.qnetwork_local(next_state).argmax(dim=1).item()
            # Use target network to get Q-value for that action
            Q_targets_next = self.qnetwork_target(next_state)[0, best_next_action]
            # Get current Q-value for the selected action
            Q_output = self.qnetwork_local(state)[0, action]

        # Compute TD target
        Q_target = reward + (self.gamma * Q_targets_next * (1 - done))
        
        # Compute TD error
        td_error = (Q_target - Q_output).item()  # Convert to scalar for single-item TD error
